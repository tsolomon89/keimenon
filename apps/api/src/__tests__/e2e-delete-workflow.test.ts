  ...
  describe('Delete Scope Variations', () => {
    test(
      ...
      `);
        ...
        canvasNode.run('node_group_1', adminAccountId, 'group', 'user', JSON.stringify(['groups']));
        canvasNode.run(
          'node_settings_1',
          adminAccountId,
          'setting',
        ...
        // Verify only canvas node deleted
        const remaining = db
          .prepare(
            `
    ...
    test(
      ...
      `);
        ...
        node.run('node_canvas_1', adminAccountId, 'message', 'user', JSON.stringify(['canvas']));
  ...
  describe('Concurrent Delete Handling', () => {
    test(
      ...
      async (_t) => {
        ...
        const { jobId: jobId1 } = await createDeleteJob('canvas', adminToken);

        // Immediately try to create second delete job (should fail or queue)
        try {
          const { jobId: jobId2 } = await createDeleteJob('canvas', adminToken);
    ...
    test(
      ...
      async (_t) => {
        ...
        // Start delete job
        const { jobId: deleteJobId } = await createDeleteJob('canvas', adminToken);
