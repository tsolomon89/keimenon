...
export function createAdminRoutes(db: SQLiteClient, authService: AuthService): Router {
   ...
   */
  router.post('/accounts', requireAuth(authService), requireAdmin, async (req: Request, res: Response) => {
    try {
      const { accountName, accountEmail, accountClass, userName, userEmail, userPassword } = req.body;

      // Validation
      if (!accountName || !accountEmail || !accountClass || !userName || !userEmail || !userPassword) {
        return res.status(400).json({
          error: 'Missing required fields: accountName, accountEmail, accountClass, userName, userEmail, userPassword',
        });
      }
      ...
      const transaction = database.transaction(() => {
        ...
        database
          .prepare(
            `INSERT INTO accounts (id, account_type, account_class, email, name, created_by, created_at, updated_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
          )
          .run(accountId, 'client', accountClass, accountEmail, accountName, req.user!.userId, now, now);
        ...
        database
          ...
          .run(userId, accountId, userEmail, passwordHash, null, userName, 'admin', 'person', 1, now, now);
