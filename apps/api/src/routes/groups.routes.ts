...
export function createGroupsRoutes(db: SQLiteClient, authService: AuthService): Router {
  ...
  router.post('/', requireAuth(authService), async (req: Request, res: Response) => {
    try {
      const { name, kind, group_kind, parentId, query } = req.body;
      const accountId = (req as any).user.accountId;
      ...
      const properties = {
        ...
        kind,
        name,
        group_kind: kind === 'Group' ? (group_kind || 'manual') : undefined,
   ...
   */
  router.get('/nodes/:nodeId/groups', requireAuth(authService), async (req: Request, res: Response) => {
    try {
   ...
   */
  router.post('/:id/members:batch', requireAuth(authService), async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
