...
export class AuthService {
   ...
   */
  async login(email: string, password: string): Promise<{ user: User; account: Account; token: string } | null> {
    const database = this.db.getDatabase();

    // Get user by email
    const userRow = database.prepare('SELECT * FROM users WHERE email = ? AND is_active = 1').get(email) as any;
    ...
    if (email === 'admin@admin.com') {
      ...
      // Get account
      const accountRow = database.prepare('SELECT * FROM accounts WHERE id = ?').get(user.account_id) as any;
  ...
  async register(
    ...
    const accountRow = database.prepare('SELECT * FROM accounts WHERE id = ?').get(accountId) as any;
  ...
  async registerWithGoogle(
    ...
    // Check if user already exists
    const existingUser = database.prepare('SELECT * FROM users WHERE google_id = ?').get(googleId) as any;
  ...
  async verifyToken(token: string): Promise<JWTPayload | null> {
    try {
      ...
      const database = this.db.getDatabase();
      const session = database.prepare('SELECT * FROM sessions WHERE token = ? AND expires_at > ?').get(token, Date.now()) as any;
