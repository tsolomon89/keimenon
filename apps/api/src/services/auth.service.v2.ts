...
export class AuthServiceV2 {
  ...
  async login(email: string, password: string): Promise<LoginResult> {
    ...
    if (accounts.length === 1) {
      ...
      return this.selectAccount(user.id, accounts[0].accountId);
    }
  ...
  async switchAccount(
    ...
    return this.selectAccount(userId, newAccountId, accountPassword);
  }
  ...
  async register(
    ...
    return this.selectAccount(userId, accountId);
  }
  ...
  /**
   * Insecure debug password reset that bypasses email links.
   * DO NOT ship to production without replacing with tokenized flow.
   */
  async debugResetPassword(
    ...
    // HACK(auth): Temporary insecure reset until proper token-based flow is ready.
