...
export interface ImportJob {
  id: string;
  type: string; // Added to correctly identify job type (import vs delete)
  fileName: string;
  fileType: 'chat' | 'document' | 'mixed' | 'unknown';
  platform?: 'chatgpt' | 'claude' | 'gemini' | 'unknown';
  status: ImportStatus;
  progress: number; // 0-100
  progressMessage?: string; // Pipeline stage message (e.g., "Parsing: 45/100 conversations")
  startedAt: number;
  completedAt?: number;
  stats: {
    nodesCreated: number;
...
export function ImportsTableCard({ onJobSelect, onJobsMultiSelect }: ImportsTableCardProps) {
  ...
  // Load deleted jobs from session storage on mount
  useEffect(() => {
    try {
      const stored = sessionStorage.getItem('keimenon_deleted_jobs');
      if (stored) {
        const parsed = JSON.parse(stored);
        if (Array.isArray(parsed)) {
          parsed.forEach((id) => deletedJobIdsRef.current.add(id));
        }
      }
    } catch (e) {
  ...
  // Check if any selected jobs have succeeded (cannot be retried)
  const selectedJobsIncludeSucceeded = useMemo(() => {
  ...
  const convertAPIJobToImportJob = useCallback((apiJob: any): ImportJob => {
    ...
    return {
      id: apiJob.id,
      type: apiJob.type, // Map type from API
      fileName,
  ...
  const getMergedJobs = useCallback((): ImportJob[] => {
    ...
    for (const op of backgroundOps) {
      ...
      if (!exists) {
        ...
        merged.push({
          id: op.id,
          type: op.type || (op.id.startsWith('del_') ? 'delete' : 'import'), // Infer type for legacy ops
  ...
  useEffect(() => {
    const handleVisibilityChange = async () => {
      if (document.visibilityState === 'visible') {
        console.log('[ImportsTable] Tab became visible, refreshing jobs...');
        ...
        try {
          ...
          if (error.name !== 'AbortError') {
            console.error('[ImportsTable] Tab focus refresh failed:', error);
          }
        }
      }
    };
  ...
  const handleDeleteSelected = async () => {
    ...
    /* 
    REMOVED: This check prevents deletion if *other* bulk actions are running, 
    or if the state is stuck. We handle specific button disabling in the UI.
    If the user can click Delete, we should try to delete.
    */
    /*
    ...
    */
    ...
    /* 
    REMOVED: Defensive checks that block deletion of "stuck" jobs.
    If a job is stuck in "being operated on" (e.g. from a failed retry), 
    we MUST allow the user to delete it to clear the state.
    */
    /*
  ...
  const executeDelete = async () => {
    ...
    // setBulkActionLoading(true); // No longer needed for delete
    ...
    try {
      ...
      const deletePromises = Array.from(jobIdsToDelete).map(async (jobId) => {
        try {
          ...
          if (!response.ok) {
            // CRITICAL FIX: Treat 404 as success (job already gone)
            // This allows cleaning up "phantom" jobs from the UI
            if (response.status === 404) {
              console.log(`[DELETE] Job ${jobId} not found (404), treating as deleted`);
      ...
      console.log('[DELETE] Setting isDeleting = false');
    ...
    <div className="bg-slate-800 border border-slate-700 rounded-lg overflow-hidden">
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-xl max-w-sm w-full p-6 space-y-4">
