...
import { GraphNode, GraphEdge as BaseGraphEdge, createSimulation, getNodeRadius } from '@keimenon/graph';
import {
  getEdgeStyle,
  getEdgeThickness,
  getEdgeOpacity,
...
export const Keimenon2D = memo(forwardRef<Keimenon2DHandle, Keimenon2DProps>(
  ({ nodes, edges, width, height, onNodeClick, onNodeDoubleClick, onSelectionChange, onEdgeHover }, ref) => {
    ...
    const drawFrame = () => {
      ...
      nodes.forEach(node => {
         ...
         ctx.strokeStyle = isSelected ? 'rgba(168, 85, 247, 1)' : (isHovered ? 'rgba(100, 116, 139, 0.8)' : 'rgba(100, 116, 139, 0.3)');
         ...
         if (transform.scale > 0.5) {
             ...
             ctx.fillText(node.id.slice(0, 8) + '...', node.x, node.y + radius + 15 / transform.scale);
         }
      });
        ...
        if (isSelecting && selectionBox) {
             ...
             nodes.forEach(n => {
                 if (n.x !== undefined && n.y !== undefined && 
                     n.x >= gMinX && n.x <= gMaxX && n.y >= gMinY && n.y <= gMaxY) {
...
function getNodeColor(node: GraphNode, isSelected: boolean, isHovered: boolean): string {
  ...
  let color = colors[node.kind] || 'rgba(100, 116, 139, 0.6)';

  const nodeWithMeta = node as GraphNode & { metadata?: { isDuplicate?: boolean; status?: string } };
  const isDuplicate = nodeWithMeta.metadata?.isDuplicate || nodeWithMeta.metadata?.status === 'duplicate';
  if (isDuplicate) {
