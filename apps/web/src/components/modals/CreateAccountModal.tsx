...
export function CreateAccountModal({ onClose, onSuccess }: CreateAccountModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4001'}/api/v1/admin/accounts`, {
        method: 'POST',
        headers: {
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">Create a new client account with an initial admin user</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4 space-y-4">
                ...
                <div>
                  <label htmlFor="accountEmail" className="block text-sm font-medium text-slate-300 mb-2">
                    Account Email
                  </label>
                  <input
                    id="accountEmail"
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4 space-y-4">
                ...
                <div>
                  <label htmlFor="userPassword" className="block text-sm font-medium text-slate-300 mb-2">
                    User Password
                  </label>
                  <input
                    id="userPassword"
                  ...
                  />
                  <p className="text-xs text-slate-500 mt-1">The user will be created with admin permissions for this account</p>
                </div>
              </div>
            </div>
          </div>
