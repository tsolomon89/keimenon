 ...
 *
 * This component walks data structures and renders them using the
 * appropriate primitives based on their shape:
 * - Array → List (with Tile or Card items)
 * - Object → Card (with Field children)
 * - Scalar (string, number, boolean) → Field
 *
 * The mode parameter provides context for HOW to render, but the
 ...
 * - list: Expects array of objects
 *
 * Usage:
 * ```tsx
 * // Dashboard mode - array of metrics
 * <Viewer
 *   mode="dashboard"
 *   data={[
 *     { title: 'Users', value: 1234, icon: Users },
 *     { title: 'Revenue', value: '$45K', icon: DollarSign },
 *   ]}
 * />
 *
 * // Settings mode - nested object
 * <Viewer
 *   mode="settings"
 *   data={{
 *     general: { language: 'en', theme: 'dark' },
 *     privacy: { analytics: true, tracking: false },
 *   }}
 * />
 *
...
export function Viewer({ mode, data, loading = false, error, className = '' }: ViewerProps) {
  ...
  if (loading) {
    return (
      <div className={`flex-1 flex items-center justify-center p-12 ${className}`.trim()}>
 ...
 * Grammar: Array → List, Objects → Card
 */
 ...
 * Grammar: Object → Card, Nested Objects → Card, Scalars → Field
 */
 ...
 * Grammar: Object → Card, Scalars → Field
 */
...
/**
 * List mode - Array of objects
 * Grammar: Array → List, Objects → Card/Tile
 */
 ...
 * Grammar: Array → List, Object → Card, Scalar → Field
 */
 ...
 * Grammar: Scalars → Field, Nested Objects → Card, Arrays → List
 */
