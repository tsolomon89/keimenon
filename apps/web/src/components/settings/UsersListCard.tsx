...
const USER_ROW_HEIGHT = 120;

// Props passed to UserRow (excluding forbidden keys: index, style, ariaAttributes)
interface UserRowProps {
  users: UserType[];
  currentUserId: string | undefined;
  canManageUsers: boolean;
  deletingId: string | null;
  onUserSelect: ((user: UserType) => void) | undefined;
  handleDelete: (userId: string, userName: string) => void;
  getPermissionColor: (level: string) => string;
}

// Virtualized user row component
function UserRow({
  index,
  style,
  users,
...
} & UserRowProps): React.ReactElement | null {
  ...
  return (
    <div style={style} className="py-1.5">
      <div
        ...
        className="bg-slate-800/50 border border-slate-700 rounded-lg p-4 hover:border-purple-500/50 hover:bg-slate-800 transition-all cursor-pointer h-full"
      >
...
export function UsersListCard({ onUserSelect, onCreateUser }: UsersListCardProps) {
  ...
  // Bug fix #28: Memoize filtered users to prevent unnecessary re-renders
  ...
  // Handle user deletion (memoized for virtualized list)
  ...
  // Permission badge styling (memoized for virtualized list)
  const getPermissionColor = useCallback((level: string) => {
    switch (level) {
      case 'admin':
    ...
    <div className="space-y-4">
      ...
      {filteredUsers.length === 0 ? (
        ...
        <div
          ...
          style={{ height: Math.min(filteredUsers.length * USER_ROW_HEIGHT, 400) }}
        >
          <List<UserRowProps>
            style={{ height: Math.min(filteredUsers.length * USER_ROW_HEIGHT, listHeight) }}
