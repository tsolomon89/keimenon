...
```json
{
  "todo-tree.general.tags": ["TODO", "FIXME", "HACK", "NOTE", "XXX", "BUG"],
...
4. mcp**docs**search_docs: "authentication" (internal patterns)
5. Synthesize report:
...
- mcp**api-testing**test_multi_tenant (isolation testing)
- mcp**database**query_nodes (data isolation verification)
...
async function generateReport() {
  ...
  const report = {
    ...
    skills: {
      ...
      average_tokens: Math.floor(skills.reduce((sum, s) => sum + s.tokens, 0) / skills.length),
      largest: skills.sort((a, b) => b.tokens - a.tokens).slice(0, 5),
    },
    total_estimated_tokens: claudemd.tokens + skills.reduce((sum, s) => sum + s.tokens, 0),
  };

  // Write report
  await fs.mkdir('.claude/metrics', { recursive: true });
  await fs.writeFile('.claude/metrics/token-usage.json', JSON.stringify(report, null, 2));
  ...
  console.log(
    `Skills (${skills.length} total): ${report.skills.total_tokens.toLocaleString()} tokens`
  );
