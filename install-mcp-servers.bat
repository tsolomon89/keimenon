@echo off
REM Install Canvas Memory MCP Servers to User Scope

echo Installing Canvas Memory MCP servers to user scope...
echo.

echo [1/6] Installing canvas-database...
claude mcp add --scope user --transport stdio canvas-database --env SQLITE_PATH=%USERPROFILE%\.canvas-memory\canvas.db -- node C:/Development/Projects/ai_convo_parser/.mcp/servers/database/index.js
...
claude mcp add --scope user --transport stdio canvas-settings-crm --env SQLITE_PATH=%USERPROFILE%\.canvas-memory\canvas.db -- node C:/Development/Projects/ai_convo_parser/.mcp/servers/settings-crm/index.js
